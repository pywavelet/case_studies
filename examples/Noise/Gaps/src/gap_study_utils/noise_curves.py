...
def noise_PSD_AE(f: np.ndarray, TDI='TDI1'):
    ...
    Spm = (3e-15) ** 2 * (1 + ((4e-4) / f) ** 2) * (1 + (f / (8e-3)) ** 4) * (1 / (2 * np.pi * f)) ** 4 * (
            2 * np.pi * f / c) ** 2
...
def generate_stationary_noise(ND: int, dt: float, psd: FrequencySeries, time_domain: bool = True) -> Union[
    TimeSeries, FrequencySeries]:

    variance_f = ND * psd.data / (4 * dt)  # Variance of stationary noise process
    variance_f[0] = variance_f[1]  # Set DC component to be the same as the next frequency
