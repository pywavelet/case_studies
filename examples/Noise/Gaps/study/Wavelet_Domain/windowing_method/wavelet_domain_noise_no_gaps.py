...
from gap_study_utils.wavelet_data_utils import bandpass_timeseries
...
def main(
    ...
    Nf = 32
    h_wavelet, psd_wavelet = compute_evolutionary_psd(signal_f_series, psd, Nf, delta_t)
    ...
    cov_matrix_wavelet = estimate_covariance_matrix(noise_wavelet_matrix, h_wavelet.data.shape[0],
                                                    h_wavelet.data.shape[1])
    ...
    SNR_estimated_wavelet = np.sqrt(np.nansum((h_wavelet * h_wavelet) / cov_matrix_wavelet))
