...
def gap_routine_nan(t, start_window, end_window, delta_t=10):
    ...
    nan_window = [
        (
            np.nan if (start_window < time < end_window)
            else 1
        )
        for time in t
    ]
...
def generate_noise_matrices(freq, variance_noise_f, w_t, delta_t, kwgs, num_samples=500):
    """
    ...
    for i in tqdm(range(num_samples)):
        np.random.seed(i)
        noise_f_iter = np.random.normal(0, np.sqrt(variance_noise_f)) + 1j * np.random.normal(0,
                                                                                              np.sqrt(variance_noise_f))
    ...
    return np.array(noise_wavelet_matrices), np.array(noise_gap_wavelet_matrices)


def compute_covariance_matrices(noise_wavelet_matrix, noise_gap_wavelet_matrix):
    """
...
def plot_wavelets(h_wavelet, cov_matrix_wavelet, h_wavelet_gap, cov_matrix_gap_wavelet, psd_wavelet,
                              noise_gap_wavelet, N_f, N_t, start_window, end_window, lobe_length, tmax):
    """
    ...
    freq_range = (0, 0.007)
    plot_wavelet_grid(cov_matrix_wavelet, time_grid=psd_wavelet.time / 60 / 60, freq_grid=psd_wavelet.freq, ax=ax[0, 0],
                      zscale="log", freq_scale="linear", absolute=False, freq_range=freq_range)
    ax[0, 0].set_title("Estimated wavelet covariance matrix")

    plot_wavelet_grid(h_wavelet.data, time_grid=h_wavelet.time / 60 / 60, freq_grid=h_wavelet.freq, ax=ax[0, 1],
    ...
    plot_wavelet_grid(h_wavelet_gap.data, time_grid=h_wavelet_gap.time / 60 / 60, freq_grid=h_wavelet_gap.freq,
                      ax=ax[1, 1], zscale="linear", freq_scale="linear", absolute=False, freq_range=freq_range,
                      )
...
def main(
    ...
    h_wavelet, psd_wavelet = wavelet_transform(signal_f_series, psd, delta_t, Nf)
    ...
    noise_wavelet_matrix, noise_gap_wavelet_matrix = generate_noise_matrices(freq, variance_noise_f, w_t, delta_t,
                                                                             kwgs={"Nf": Nf})
    ...
    h_wavelet_gap, psd_gap_wavelet = wavelet_transform(FrequencySeries(data=h_f_gap, freq=freq), psd, delta_t, Nf)
    SNR_gap_wavelet = np.nansum( ((h_wavelet_gap * h_wavelet_gap) / psd_gap_wavelet).data)
